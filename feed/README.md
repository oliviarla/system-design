...
### 배치 서비스
- 팔로잉/팔로워 카운트 동기화
  - 주기적으로 ScyllaDB User 테이블의 카운트 팔로잉/팔로워 카운트 컬럼과 Redis의 팔로잉/팔로워 카운트 아이템을 동기화한다.
  - 유저 서비스에서 팔로우 / 팔로우 취소 발생 시점에 Redis의 카운트를 갱신한다. 만약 Redis 요청에 실패하면 Kafka에 메시지를 전달한다.
  - 배치 서비스에서는 매일 자정마다 다음 작업을 수행한다.
    - Kafka에 쌓인 메시지가 있다면, 각 유저(source, target)의 팔로잉/팔로워 카운트를 ScyllaDB에서 계산한 후 Redis 아이템을 갱신한다.
    - Redis의 카운트를 순회하면서 ScyllaDB User 테이블의 카운트와 일치하지 않는 경우 테이블의 카운트를 갱신한다.
- 뉴스 피드 아이템 크기 제한

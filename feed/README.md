...
- 피드를 발행할 수 있다.
  ...
  - 특정 회원이 작성한 최신 피드의 목록은 Redis ZSET에 캐싱한다.
    ```
    - key: "feed:{userId}"
    - score: feedId
    - member: feedId
...
- 뉴스 피드 데이터 구축
  - **뉴스 피드 캐시 아이템 자체가 없을 때**
    ...
    - 뉴스 피드 아이템 생성 과정의 전체적인 그림은 다음과 같다.
      <img alt="image" src="https://github.com/user-attachments/assets/bcf9b666-7f74-4ba8-b4d9-2d6198d3ed99" />
    - 실제로는 다음과 같은 방식으로 진행된다.
      ...
      5. 각 피드 아이디에 대한 정보를 조회한다.
          - 뉴스 피드 ID 목록들에 대하여 DB에서 상세 정보를 조회한다.
  - **뉴스 피드 캐시 아이템이 존재할 때**
      - 뉴스 피드 캐시 아이템이 있더라도 팔로워가 매우 많은 인플루언서를 팔로우하고 있다면, fan-out 과정에서 뉴스 피드 캐시에 피드가 추가되지 않는다.
      - 따라서 사용자가 맨 첫 페이지를 요청했거나, `ZREVRANGE` 결과가 `size` 인자보다 작을 때 fan-in을 통해 뉴스피드 갱신 작업을 진행한다.
      - 뉴스 피드 갱신 과정은 다음과 같다.
        1. 사용자의 팔로잉 목록 중 팔로워가 매우 많은 인플루언서를 필터링한다. 이 때에도 팔로잉하는 인플루언서 수가 1000 이상인 경우 100개씩 청크 단위로 나누어 팔로잉 유저 목록을 조회하고, 그 외의 경우 모든 팔로잉 유저 목록을 조회한다.
